...
for epoch in range(NUM_EPOCHS):
        ...
        for step, (inputs, labels) in enumerate(train_loader):
            ...
            print('Train Batch Loss: {:.4f} Train Batch Softmax Loss: {:.4f} Train Batch Center Loss: {:.4f} Acc: {:.4f} Elapsed: {:.0f}m {:.0f}s'.format(loss.data.item(), loss_softmax.data.item(), loss_center.data.item() * LR_CENTER, torch.sum(preds == labels.data).double() / inputs.data.size(0), time_elapsed // 60, time_elapsed % 60))
        ...
        epoch_loss = running_loss / limport torch
import torch.nn as nn
import torch.optim as optim
from torch.optim import lr_scheduler
import torchvision
